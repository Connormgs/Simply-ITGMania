...
for i=1,NumEntries do
	...
	if i == 1 then
		...
		af[#af+1] = LoadFont("Common Normal")..{
			...
			Text="",
			InitCommand=function(self)
				self:diffuse(Color.White):xy(-width/2 + 27, y):maxwidth(30):horizalign(right):zoom(0.6)
				if IsNotWide and #GAMESTATE:GetHumanPlayers() > 1 then
					self:x(-width/2 + 42)
				end
			end,
	...
	af[#af+1] = LoadFont("Common Normal")..{
		...
		local height = 28
			local spacing = 2
			self:diffuse(Color.White):xy(-width/2 + 30, y):maxwidth(100):horizalign(left):zoom(0.6)
			if IsNotWide and #GAMESTATE:GetHumanPlayers() > 1 then
	...
	af[#af+1] = LoadFont("Common Normal")..{
		...
		Text="",
		InitCommand=function(self)
			self:diffuse(Color.White):xy(-width/2 + 160, y):horizalign(right):zoom(0.55)
			if IsNotWide and #GAMESTATE:GetHumanPlayers() > 1 then
				self:x(-width/2 + 140)
			end
		end,
